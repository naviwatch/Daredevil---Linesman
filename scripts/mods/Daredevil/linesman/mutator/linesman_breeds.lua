...
mod:hook_origin(BeastmenStandardHealthExtension, "add_damage",
    function(self, attacker_unit, damage_amount, hit_zone_name, damage_type, hit_position, damage_direction,
             damage_source_name, hit_ragdoll_actor, damaging_unit, hit_react_type, is_critical_strike, added_dot,
             first_hit, total_hits, attack_type)
        if mutator_plus.active then
            if damage_source_name == "suicide" then
                BeastmenStandardHealthExtension.super.add_damage(self, attacker_unit, damage_amount, hit_zone_name,
                    damage_type, hit_position, damage_direction, damage_source_name, hit_ragdoll_actor, damaging_unit,
            ...
            if damage_source_name == "suicide" then
                ...
                can_damage_banner = attack_type and (attack_type == "heavy_attack" or attack_type == "light_attack") or
                white_listed_damage_sources[damage_source_name]
...
mod:hook(BeastmenStandardExtension, "on_death", function(func, self, killer_unit)
    if Unit.alive(killer_unit) and killer_unit ~= self.unit then
        ...
        if self.is_server then
            Managers.state.entity:system("surrounding_aware_system"):add_system_event(self.unit,
                "standard_bearer_buff_deactivated", DialogueSettings.special_proximity_distance_heard)
        end
    else
...
-- Fucking blights
BreedActions.chaos_vortex_sorcerer.skulk_approach = {
		close_distance = 10,
		far_away_from_target_sq = 625, -- this is apparently useless
		max_cast_vortex_distance = 25,
		max_player_vortex_distance = 0,
