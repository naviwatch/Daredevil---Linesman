...
-- New surge stuff
local surge_timer_range = {5, 10}          -- Range for special respawn timers during surge (seconds)
local surge_duration = 30                  -- How long the surge lasts (seconds)
local chance_of_surge = 0.4                -- Probability of surge triggering during coordinated attack
...
local stagger_time = 2                     -- Stagger time between same-breed spawns (seconds)

-- Horde-spawning specials
local horde_spawn_specials = 0.25           -- Probability of a slot being turned into a "horde special", ie spawned using horde mechanics
        ...
        if slot.state == "waiting" then
            if t > slot.time then
                if mutator_plus.active and not use_fixed_cooldown and Math.random() <= horde_spawn_specials then -- 30% chance for non-fixed cooldown slots
                    ...
                    slot.time = t + ConflictUtils.random_interval(cooldown_range) -- Reset timer
                else
    ...
    if mutator_plus.active and #slots_to_spawn > 0 then
        ...
        for breed, group in pairs(breed_groups) do
            if #group > 1 and Math.random() <= same_breed_chance then -- 30% chance to group
                ...
                local base_time = t + 5 -- Base spawn time for the group
